...
def get_flow_day(cursor_op):
    sql = """
    SELECT date_trunc('hour', MIN(start_time))
FROM flow    """

    cursor_op.execute(sql)
    return cursor_op.fetchone()[0]

def get_max_date(cursor_op):
    sql = """
SELECT date_trunc('hour', MAX(start_time))
    FROM flow
    """
...
def fill_dimDay(cursor_dwh, flow_date, max_date):
    sql = """
    ...
    max = pd.to_datetime(max_date)
    records_inserted = 0

    with tqdm (total=(max - date).days) as pbar:
        while date <= max:
...
def main():
    try:
        ...
        max_date = get_max_date(cursor_op)

        fill_dimDay(cursor_dwh, flow_date, max_date)
